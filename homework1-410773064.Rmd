...
```{r ans13}
meanEcon0 <- fromEcon*grade
meanEcon <-sum(fromEcon*grade)/2
meanEcon
```
...
### 2.1
請將submitTime轉成電腦看得懂的date time並存回相同答案物件ymd_hms。(原資料來自Google表單，所以是內定的UTC時區)
```{r ans21}
...
```{r ans24}
sumnightOwl <- sum(nightOwl)
nightOwlVoteShare <- sumnightOwl/length(hour)
nightOwlVoteShare
```
...
```{r ans32}
credit100ChosenStudent0 <- studentID == "046S6f645" & schoolYear == 100
credit100ChosenStudent <- sum(credit100ChosenStudent0*credit)
...
```{r ans33}
earnedCredit100ChosenStudent0 <- studentID == "046S6f645" & schoolYear == 100 & grade >= 60
earnedCredit100ChosenStudent <- sum(earnedCredit100ChosenStudent0*credit)
...
```{r ans34}
gpa100ChosenStudent <- sum(earnedCredit100ChosenStudent0*grade*credit)/sum(credit100ChosenStudent0*credit)
gpa100ChosenStudent
```
